  ...
  options = {
    ...
    yAxis: [
      ...
      axisLine: {
        show: true,
        lineStyle: {
          color: 'rgb(0,114,178)', //Neo4j
          // color: 'rgb(0,158,115)', //postgres
          // color: 'rgb(0,0,0)', //duckdb
          width: 5,
          type: 'solid'
        }
      }
    },
    {
      ...
      axisLine: {
        show: true,
        lineStyle: {
          color: 'rgb(213,94,0)', //Neo4j
          // color: 'rgb(204,121,167)', //postgres
          // color: 'rgb(230,159,0)', //duckdb
          width: 5,
          type: 'solid'
        }
      }
...
function formatMedianLineChart(results){
  ...
  results.data.forEach(res => {
    if (!element_position.has(res["order"])) {
      ...
      element_position.set(res["order"], unformatted.length - 1)
    }

    elementPos = element_position.get(res["order"])
    unformatted[elementPos].totalRuns += 1

    if (res["query execution time"] !== "timeout") {
      unformatted[elementPos].execTimes.push(parseInt(res["query execution time"]))
    }
  });
  ...
  unformatted.forEach(el => {
    el.execTimes.sort((a, b) => a - b)
    if (el.execTimes.length < el.totalRuns/2) {
      median = 350000
    } else if (el.execTimes.length == 1) {
      median = el.execTimes[0]
    } else if (el.execTimes.length == 2) {
      median = (el.execTimes[0] + el.execTimes[1]) / 2
    } else if (el.execTimes.length % 2 == 1) {
      median = el.execTimes[(el.execTimes.length + 1) / 2]
    } else {
      median = (el.execTimes[el.execTimes.length / 2] + el.execTimes[(el.execTimes.length / 2) + 1]) / 2
    }
...
function formatTimeoutBarChart(results){
  ...
  results.data.forEach(res => {
    ...
    elementPos = element_position.get(res["order"])
    unformatted[elementPos].total_runs += 1

    if (res["query execution time"] == "timeout") {
      unformatted[elementPos].timeouts +=1
    }
  });
  ...
  //     formatted[elementPos] = formatted[elementPos]-10
  //   }
  // })

  formatted = []
  unformatted.forEach(el => {
    formatted.push((100*el.timeouts)/el.total_runs)
  })
